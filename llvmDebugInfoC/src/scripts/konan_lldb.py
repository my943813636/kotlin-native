...
class KonanObjectSyntheticProvider(KonanHelperProvider):
    def __init__(self, valobj, tip):
        ...
        if not tip in SYNTHETIC_OBJECT_CACHE:
            SYNTHETIC_OBJECT_CACHE[tip] = [
                ChildMetaInfo(self._field_name(i), self._field_type(i), self._field_address(i) - self._valobj.unsigned)
                for i in range(self._children_count)]
            log("TIP: {:#x} MISSED".format(tip))
        else:
            log("TIP: {:#x} HIT".format(tip))
        self._children = SYNTHETIC_OBJECT_CACHE[tip]
...
class KonanArraySyntheticProvider(KonanHelperProvider):
    def __init__(self, valobj):
        ...
        self._children = [ChildMetaInfo(str(x), type, offset + x * size) for x in range(self.num_children())]
